...
export const calculateMonthlyPayroll = functionsV2.onCall(async (request) => {
  const { auth, data } = request;
  if (!auth) {
    throw new functionsV2.HttpsError('unauthenticated', 'User must be logged in');
  }

  const { companyId, month, year, actorId, actorName } = data;
  if (!companyId || !month || !year) {
    throw new functionsV2.HttpsError('invalid-argument', 'Missing required fields');
  }
  ...
  try {
    ...
    await db.runTransaction(async (t) => {
      ...
      if (snap.exists) {
        ...
        if (cycle && ['PROCESSING', 'APPROVED', 'LOCKED', 'DISBURSED'].includes(cycle.status)) {
          throw new functionsV2.HttpsError('failed-precondition', `Cycle is currently ${cycle?.status}. Cannot recalculate.`);
        }
      }
      t.set(cycleRef, {
    ...
    await cycleRef.update({
      ...
      totalDeductions, totalNetPay, processedAt: new Date().toISOString(), processedBy: actorId || auth.uid, processedByName: actorName || 'System'
    });
    ...
    await auditRef.set({
      ...
      userId: actorId || auth.uid, userName: actorName || 'System',
      details: `Calculated payroll for ${employees.length} employees for ${month}/${year} using dynamic statutory state rules (Cloud Function)`,
    ...
    await cycleRef.update({ status: 'DRAFT' });
    throw new functionsV2.HttpsError('internal', error.message || 'Error calculating payroll');
  }
});
