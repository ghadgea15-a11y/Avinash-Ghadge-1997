...
// We need to inject imports if needed, but TaskRecord, AnnouncementRecord, DocumentRecord are exported from index.ts. 
// So we can just add the functions inside FirestoreService class.
...
content = content.replace(
  "import {", 
  "import { TaskRecord, AnnouncementRecord, DocumentRecord,"
);

const idx = content.lastIndexOf("}");
content = content.substring(0, idx) + methods + content.substring(idx);

fs.writeFileSync(filePath, content);
console.log('FirestoreService updated.');
