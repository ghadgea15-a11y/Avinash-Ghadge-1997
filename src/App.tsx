...
import { SuperAdminLandingEditorScreen } from "./components/screens/SuperAdminLandingEditorScreen";
...
import { SupportImpersonationBanner } from "./components/common/SupportImpersonationBanner";
import { MaintenanceGateScreen } from "./components/common/MaintenanceGateScreen";
import { SuperAdminService } from "./services/superAdminService";
import { PlatformGlobalConfig } from "./types/platform";
import { Loader2, ShieldAlert } from "lucide-react";
import { isScreenAllowedForCompany } from "./config/navigationArchitecture";
...
function MainApp() {
  ...
  const [activeCompany, setActiveCompany] = useState<CompanyTenant | null>(
    null,
  );
  ...
  const [viewportMode, setViewportMode] = useState<
    "PHONE" | "TABLET" | "FULLSCREEN"
  >("PHONE");
  const [offlineQueueCount, setOfflineQueueCount] = useState<number>(
    OfflineSyncService.getQueue().length,
  );
  const [isDrawerOpen, setIsDrawerOpen] = useState<boolean>(false);
  const [unreadNotifCount, setUnreadNotifCount] = useState<number>(0);

  // Platform Global Config for Maintenance Mode Enforcement
  const [platformConfig, setPlatformConfig] = useState<PlatformGlobalConfig | null>(null);
  ...
  const isSuperAdminSession = 
    ...
    currentScreen.startsWith("SUPER_ADMIN_") ||
    currentScreen === "PLATFORM_LOGIN";

  const isMaintenanceBlocked = 
    Boolean(platformConfig?.maintenanceMode) && !isSuperAdminSession;
    ...
    if (
      ...
      if (!activeCompany || activeCompany.companyId !== userSession.companyId) {
        FirebaseAuthService.verifyCompanyCode(userSession.companyId)
          ...
          .catch((err) => {
            console.warn(
              "[App] Failed to auto-load company tenant for active session:",
              err,
            );
          });
      }
          ...
          if (uData.role && uData.role !== userSession.role) {
            FirebaseAuthService.refreshSession(userSession).then((updated) => {
              if (updated) setUserSession(updated);
                ...
                setUserSession((prev) =>
                  prev ? { ...prev, role: uData.role } : null,
                );
            });
          }
        }
          ...
          if (mData.role && mData.role !== userSession.role) {
            FirebaseAuthService.refreshSession(userSession).then((updated) => {
              if (updated) setUserSession(updated);
                ...
                setUserSession((prev) =>
                  prev ? { ...prev, role: mData.role } : null,
                );
            });
          }
          if (mData.siteId && mData.siteId !== userSession.assignedSiteId) {
            setUserSession((prev) =>
              prev ? { ...prev, assignedSiteId: mData.siteId } : null,
            );
          }
          if (
            mData.departmentId &&
            mData.departmentId !== userSession.departmentId
          ) {
            setUserSession((prev) =>
              prev ? { ...prev, departmentId: mData.departmentId } : null,
            );
          }
        }
      },
        ...
        {currentScreen === "LANDING" ? (
          ...
          <div className="min-h-screen w-full bg-slate-950 text-white p-4">
            <div className="max-w-5xl mx-auto">
              <KotlinCodeViewer
                onBack={() =>
                  setCurrentScreen(userSession ? "EMPLOYEES" : "LANDING")
                }
              />
            </div>
          </div>
          ...
          <div
            style={
              activeCompany?.loginBackgroundUrl
                ? {
                    backgroundImage: `url(${activeCompany.loginBackgroundUrl})`,
          ...
          /* Real-time Global Platform Maintenance Lockdown Gate */
          <MaintenanceGateScreen
            ...
            onNavigateToSuperAdminLogin={() => {
              handleLogout();
              setCurrentScreen("PLATFORM_LOGIN");
            }}
            onRecheck={() => {
          ...
          <div className="min-h-screen flex flex-col font-sans transition-colors duration-200">
            {/* Top Controlled Support Impersonation Banner */}
            <SupportImpersonationBanner
              userSession={userSession}
              onExit={() => {
                SessionManager.clearSupportImpersonation();
                if (userSession?.role === "SUPER_ADMIN" || userSession?.companyId === "GLOBAL_ADMIN") {
                  setActiveCompany(null);
                  SessionManager.clearActiveCompany();
                  setCurrentScreen("SUPER_ADMIN_SUPPORT");
                }
              }}
                ...
                <div className="flex-1 flex flex-row overflow-hidden w-full h-full relative">
                  ...
                  <div className="flex-1 flex flex-col justify-between overflow-hidden relative w-full">
                    <div className="flex-1 overflow-y-auto w-full">
                      ...
                      !isScreenAllowedForCompany(currentScreen, activeCompany).allowed ? (
                        <div className="flex flex-col items-center justify-center min-h-[500px] p-8 text-center max-w-lg mx-auto">
                          <div className="w-16 h-16 rounded-2xl bg-amber-500/10 border border-amber-500/20 text-amber-600 dark:text-amber-400 flex items-center justify-center mb-4 shadow-sm">
                            <ShieldAlert className="w-8 h-8" />
                          </div>
                          <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-red-100 text-red-700 dark:bg-red-950/60 dark:text-red-400 mb-3 tracking-wide uppercase">
                            Access Restricted
                          </span>
                          <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-2">
                            Module Disabled for {activeCompany?.name || 'Organization'}
                          </h2>
                          <p className="text-sm text-slate-600 dark:text-slate-400 mb-6 leading-relaxed">
                            Access to the <strong>{isScreenAllowedForCompany(currentScreen, activeCompany).label || isScreenAllowedForCompany(currentScreen, activeCompany).moduleKey}</strong> module has been disabled for your organization in the Super Admin Entitlements & Access Control console.
                          </p>
                          <button
                            onClick={() => setCurrentScreen('ENTERPRISE_DASHBOARD')}
                            className="inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm font-medium transition-colors shadow-sm cursor-pointer"
                          >
                        ...
                        >
                          ...
                          {currentScreen === "SUPER_ADMIN_SUPPORT" && (
                            <SuperAdminSupportScreen
                              ...
                              onImpersonateSuccess={(company) => {
                                setActiveCompany(company as any);
                                SessionManager.setActiveCompany(company as any);
                                setCurrentScreen("ENTERPRISE_DASHBOARD");
                              }}
                            />
                          )}
                          ...
                          {currentScreen === "SUPER_ADMIN_LANDING_EDITOR" && (
                            <SuperAdminLandingEditorScreen
                              currentSession={userSession!}
                              onNavigate={(screen: any) =>
                                setCurrentScreen(screen)
                              }
                      ...
                      {currentScreen === "ENTERPRISE_DASHBOARD" &&
                        (activeCompany ? (
                          ...
                          <div
                            id="company-resolving-loader"
                            className="flex-1 flex items-center justify-center p-12 min-h-[400px]"
                          >
                      ...
                      {currentScreen === "DOCUMENT_LIFECYCLE" &&
                        activeCompany && (
                      ...
                      {currentScreen === "OPERATIONAL_INTELLIGENCE" &&
                        activeCompany && (
                      ...
                      {currentScreen === "HISTORICAL_TRACEABILITY" && (
                        <HistoricalTraceabilityScreen
                          session={userSession}
                          onNavigateToScreen={(scr: any) =>
                            setCurrentScreen(scr as PhaseAScreen)
                          }
                        />
                      )}
                      ...
                      {currentScreen === "TALENT_MANAGEMENT" && (
                        <TalentManagementScreen session={userSession} />
                      )}
                      ...
                      {(currentScreen === "GENERAL_LEDGER" ||
                        currentScreen === "FINANCE_DASHBOARD") &&
                        activeCompany && (
                      ...
                      {currentScreen === "ENTERPRISE_INTEGRATIONS" &&
                        activeCompany && (
                      ...
                      {(currentScreen === "GUARD_PATROL" || (currentScreen as string) === "MULTI_MODE_PATROL") && (
                        <MultiModePatrolScreen
                          companyId={activeCompany?.id || (activeCompany as any)?.companyId || ''}
                      ...
                      {currentScreen === "COMPLIANCE_EXPIRY" &&
                        activeCompany && (
                      ...
                      {(currentScreen === "BIOMETRIC_DEVICES" ||
                        currentScreen === "DEVICE_INTEGRATION_HUB") && (
                        <BiometricHubScreen
                          userSession={userSession}
                          activeCompany={activeCompany as any}
                          onNavigate={(screen: any) => setCurrentScreen(screen)}
