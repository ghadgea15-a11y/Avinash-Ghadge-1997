...
export const AttendanceAdjustmentWorkflow: React.FC<Props> = ({ userSession, companyId }) => {
  ...
  if (loading) return <div className="p-4 text-slate-500">Loading exceptions...</div>;
  ...
  return (
    <div className="space-y-4">
      ...
      {exceptions.length === 0 ? (
        <div className="bg-slate-50 border border-slate-200 rounded-xl p-8 text-center text-slate-500 flex flex-col items-center">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-lg shadow-xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-6 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
              <h2 className="text-xl font-bold text-slate-900 dark:text-white">Resolve Attendance</h2>
              <button onClick={() => setSelectedAtt(null)} className="p-2 text-slate-400 hover:text-slate-600 rounded-full hover:bg-slate-100">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="p-6 overflow-y-auto space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 mb-1">Approved OT (Mins)</label>
                  <input 
                    type="number"
                    value={approvedOt}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 mb-1">Reason for Change (Required)</label>
                <textarea 
                  value={reason}
            ...
            <div className="p-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 flex justify-end gap-3">
              <button 
                ...
                className="px-4 py-2 font-bold text-slate-600 bg-white border border-slate-200 rounded-lg hover:bg-slate-50"
              >
                Cancel
              </button>
              <button 
