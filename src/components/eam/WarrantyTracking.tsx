    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {warranties.map(w => (
            <div key={w.id} className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6 flex flex-col hover:shadow-md transition-shadow">
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-bold text-slate-900 dark:text-white">{w.warrantyNumber}</h3>
                    <p className="text-xs text-slate-500">Asset: {assets.find(a => a.id === w.assetId)?.assetName || w.assetId}</p>
                  </div>
                </div>
              ...
              <div className="space-y-3 mb-6 flex-1">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium text-slate-900 dark:text-white">{w.warrantyProvider || '-'}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-slate-500">Start Date</span>
                  <span className="font-medium text-slate-900 dark:text-white">{new Date(w.startDate).toLocaleDateString()}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-slate-500">End Date</span>
                  <span className="font-medium text-slate-900 dark:text-white">{new Date(w.endDate).toLocaleDateString()}</span>
                </div>
              </div>
              ...
              <div className="pt-4 border-t border-slate-100 dark:border-slate-700 mt-auto">
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 rounded-lg text-sm font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
          ...
          {warranties.length === 0 && (
            <div className="col-span-full flex flex-col items-center justify-center p-12 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-dashed border-slate-300 dark:border-slate-700">
              ...
              <h3 className="text-lg font-medium text-slate-900 dark:text-white mb-1">No Warranties</h3>
              ...
              <div className="flex flex-col md:flex-row justify-between gap-4">
                <div>
                  <div className="flex items-center gap-3 mb-1">
                    <h3 className="font-bold text-slate-900 dark:text-white">{claim.id}</h3>
                    <span className="px-2.5 py-0.5 rounded-full bg-slate-100 text-slate-700 text-xs font-medium">
                      {claim.status}
                    </span>
                  </div>
                  <p className="text-sm text-slate-500 mb-3">Asset: {assets.find(a => a.id === claim.assetId)?.assetName || claim.assetId}</p>
                  <p className="text-slate-700 dark:text-slate-300">{claim.issueDescription}</p>
                </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
            ...
            >
              <div className="sticky top-0 z-10 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 p-6 flex justify-between items-center">
                <h2 className="text-xl font-bold text-slate-900 dark:text-white">Register Warranty</h2>
                <button onClick={() => setIsWarrantyModalOpen(false)} className="text-slate-400 hover:text-slate-600">
              ...
              <form onSubmit={handleSaveWarranty} className="p-6 space-y-6">
                <div className="grid grid-cols-2 gap-6">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Warranty Number</label>
                    <input
                      required
                      type="text"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Provider/Vendor</label>
                    <input
                      type="text"
                  ...
                  <div className="col-span-2">
                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Coverage Description</label>
                    <textarea
                      rows={3}
                  ...
                  <div className="col-span-2">
                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Warranty Documents</label>
                    <input
                      type="file"
                      multiple
                    ...
                    {warrantyFiles.length > 0 && (
                      <p className="text-xs text-slate-500 mt-2">{warrantyFiles.length} file(s) selected</p>
                    )}
                  </div>
                </div>
                <div className="flex justify-end gap-3 pt-4">
                  <button type="button" onClick={() => setIsWarrantyModalOpen(false)} className="px-4 py-2 text-slate-600">Cancel</button>
        ...
        {isClaimModalOpen && selectedWarranty && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1 }}
              exit={{ opacity: 0, scale: 0.95 }}
              className="bg-white dark:bg-slate-800 rounded-xl shadow-xl w-full max-w-xl max-h-[90vh] overflow-y-auto"
            >
              <div className="sticky top-0 z-10 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 p-6 flex justify-between items-center">
                <h2 className="text-xl font-bold text-slate-900 dark:text-white">File Warranty Claim</h2>
                <button onClick={() => { setIsClaimModalOpen(false); setSelectedWarranty(null); }} className="text-slate-400 hover:text-slate-600">
                  <XCircle className="w-6 h-6" />
                </button>
              </div>
              <form onSubmit={handleCreateClaim} className="p-6 space-y-6">
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Warranty Details</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Evidence Documents</label>
                  <input
                    type="file"
                    multiple
                    ref={claimFileInput}
                  ...
                  />
                  {claimFiles.length > 0 && (
                    <p className="text-xs text-slate-500 mt-2">{claimFiles.length} file(s) selected</p>
                  )}
                </div>
                <div>
                ...
                <div className="flex justify-end gap-3 pt-4">
                  <button type="button" onClick={() => { setIsClaimModalOpen(false); setSelectedWarranty(null); }} className="px-4 py-2 text-slate-600">Cancel</button>
                  <button type="submit" disabled={uploading} className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg disabled:opacity-50">
