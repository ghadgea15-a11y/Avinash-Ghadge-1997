...
export function AssetIncidentHistory({ session, companyId, asset, sites, onClose }: AssetIncidentHistoryProps) {
  ...
  const getStatusColor = (status: string) => {
    switch(status) {
      case 'CLOSED': return 'bg-slate-100 text-slate-900';
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
      <motion.div
        ...
        className="bg-white dark:bg-slate-900 dark:bg-slate-800 rounded-xl shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden"
      >
        <div className="bg-white dark:bg-slate-900 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 p-6 flex justify-between items-center">
          <div>
            <h2 className="text-xl font-bold text-black dark:text-white">Incident History</h2>
        ...
        <div className="flex-1 overflow-auto p-6 flex gap-6 flex-col md:flex-row">
          ...
          <div className="w-full md:w-1/3 border-r border-slate-200 dark:border-slate-700 pr-6 space-y-3 h-full overflow-y-auto">
            <h3 className="font-semibold text-black dark:text-white mb-4">Recorded Incidents</h3>
            {loading ? (
              <p className="text-sm text-slate-500 dark:text-slate-400">Loading...</p>
            ) : incidents.length === 0 ? (
              <p className="text-sm text-slate-500 dark:text-slate-400 bg-white dark:bg-slate-950 p-4 rounded-lg border border-slate-100">No incidents recorded for this asset.</p>
            ) : (
              incidents.map(inc => (
                <div 
                  key={inc.id}
                  ...
                  className={`p-3 rounded-lg border cursor-pointer transition-colors ${
                    selectedIncident?.id === inc.id 
                      ...
                      : 'bg-white border-slate-200 hover:bg-white dark:bg-slate-800 dark:border-slate-700'
                  }`}
                >
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400">{new Date(inc.reportedAt).toLocaleDateString()}</p>
                </div>
              ))
            )}
          </div>
          ...
          <div className="w-full md:w-2/3 h-full overflow-y-auto pl-2 pr-2">
            {!selectedIncident ? (
              ...
              <div className="space-y-6 pb-6">
                <div className="bg-white dark:bg-slate-950 dark:bg-slate-900 rounded-lg p-4 border border-slate-200 dark:border-slate-700">
                  <h3 className="font-bold text-lg mb-2">{selectedIncident.title}</h3>
                  <div className="grid grid-cols-2 gap-4 text-sm mb-4">
                    <div><span className="text-slate-500 dark:text-slate-400">Reported By:</span> {selectedIncident.reportedByName}</div>
                    <div><span className="text-slate-500 dark:text-slate-400">Date:</span> {new Date(selectedIncident.reportedAt).toLocaleString()}</div>
                    <div><span className="text-slate-500 dark:text-slate-400">Type:</span> {selectedIncident.lossDamageType}</div>
                    <div><span className="text-slate-500 dark:text-slate-400">Severity:</span> {selectedIncident.severity}</div>
                  ...
                  <p className="text-sm text-slate-900 dark:text-slate-300 whitespace-pre-wrap">{selectedIncident.description}</p>
                </div>
                ...
                {selectedIncident.status !== 'CLOSED' && (
                  <>
                    ...
                    <div className="bg-white dark:bg-slate-900 border border-slate-200 rounded-lg p-4 space-y-4">
                      <h4 className="font-semibold text-black dark:text-slate-200 mb-3">Investigation & Resolution</h4>
                      
                      <div>
                        <label className="block text-sm font-medium text-slate-900 dark:text-slate-300 mb-1">Findings / Root Cause</label>
                        <textarea
                          rows={3}
                          value={findings}
                      ...
                      <div>
                        <label className="block text-sm font-medium text-slate-900 dark:text-slate-300 mb-1">EAM Resolution State</label>
                        <select
                          value={eamResolution}
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        />
                        <label htmlFor="closeIncident" className="text-sm font-medium text-slate-900 dark:text-slate-300">Close Incident</label>
                      </div>
                ...
                {selectedIncident.status === 'CLOSED' && (
                  <div className="bg-slate-100 p-4 rounded-lg border border-slate-200 text-center">
                    ...
                    <h4 className="font-bold text-black dark:text-white">Incident Closed</h4>
                    <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Resolution: {selectedIncident.eamResolution}</p>
