      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <button 
            ...
            className="text-[10px] uppercase tracking-wider text-slate-500 hover:text-amber-500 font-bold mb-1 block"
          >
            ← Back to Dashboard
          </button>
                ...
                <button
                  ...
                  className={`w-full text-left p-3 rounded-xl border transition-all flex items-center justify-between gap-4 ${
                    selectedLead?.id === lead.id 
                      ...
                      : isDark ? 'bg-transparent border-transparent hover:bg-slate-800/50' : 'bg-transparent border-transparent hover:bg-slate-50'
                  }`}
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      <span className="font-bold text-sm truncate pr-2 text-slate-900 dark:text-white">{lead.company}</span>
        ...
        <div className={`rounded-2xl border flex flex-col h-[600px] overflow-hidden ${isDark ? 'bg-slate-900/80 border-slate-800' : 'bg-white border-slate-200 shadow-sm'}`}>
          {selectedLead ? (
            <div className="flex-1 flex flex-col h-full">
              ...
              <div className="p-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/50">
                ...
                <div className="space-y-2 text-xs text-slate-600 dark:text-slate-400">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium text-slate-900 dark:text-slate-200">{selectedLead.name}</span>
                  </div>
                  <div className="flex items-center gap-2">
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-6">
                ...
                <div className="space-y-2">
                  <h3 className="text-xs font-bold uppercase tracking-wider text-slate-500">Requirements & Context</h3>
                  <div className={`p-3 rounded-xl border text-xs space-y-2 ${isDark ? 'bg-slate-950/50 border-slate-800' : 'bg-slate-50 border-slate-200'}`}>
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        <span className="text-slate-500 block text-[10px]">Workforce Size</span>
                        <span className="font-medium">{selectedLead.workforceSize || 'Not specified'}</span>
                      </div>
                      <div>
                        <span className="text-slate-500 block text-[10px]">Interested In</span>
                        <span className="font-medium">{selectedLead.interestedModules || 'General'}</span>
                      </div>
                    </div>
                    {selectedLead.message && (
                      <div className="pt-2 border-t border-slate-200 dark:border-slate-800">
                        <span className="text-slate-500 block text-[10px] mb-1">Message</span>
                        <p className="text-slate-700 dark:text-slate-300 whitespace-pre-wrap">{selectedLead.message}</p>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        className={`flex-1 px-3 py-2 rounded-xl text-xs border focus:outline-none ${
                          isDark ? 'bg-slate-950 border-slate-800 focus:border-emerald-500' : 'bg-slate-50 border-slate-200 focus:border-emerald-500'
                        }`}
                        onKeyDown={(e) => {
                    ...
                    <div className="space-y-4 pt-2 relative before:absolute before:inset-0 before:ml-[11px] before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-slate-200 dark:before:via-slate-800 before:to-transparent">
                      {(selectedLead.activityHistory || []).slice().reverse().map((act) => (
                        <div key={act.id} className="relative flex items-start gap-3">
                          ...
                          <div className={`flex-1 p-2.5 rounded-xl border text-xs ${isDark ? 'bg-slate-950/50 border-slate-800' : 'bg-slate-50 border-slate-200'}`}>
                            <div className="flex justify-between items-start mb-1 gap-2">
                              <span className="font-medium text-slate-900 dark:text-slate-200">{act.actorName || 'System'}</span>
                              <span className="text-[9px] text-slate-500 shrink-0">{new Date(act.timestamp).toLocaleString()}</span>
                            </div>
                      ...
                      <div className="relative flex items-start gap-3">
                        ...
                        <div className={`flex-1 p-2.5 rounded-xl border text-xs ${isDark ? 'bg-slate-950/50 border-slate-800' : 'bg-slate-50 border-slate-200'}`}>
                          <div className="flex justify-between items-start mb-1 gap-2">
                            <span className="font-medium text-slate-900 dark:text-slate-200">System</span>
                            <span className="text-[9px] text-slate-500 shrink-0">{new Date(selectedLead.createdAt).toLocaleString()}</span>
                          </div>
            ...
            <div className="h-full flex flex-col items-center justify-center text-slate-400 p-8 text-center space-y-4">
              <UserCheck className="w-12 h-12 text-slate-300 dark:text-slate-700" />
              <div>
                <p className="font-medium text-sm text-slate-500">No Lead Selected</p>
