...
export const RegionalAreaManagerDashboard: React.FC<DashboardProps> = ({ userSession, company, onNavigate }) => {
  ...
  useEffect(() => {
    const unsubEmployees = FirestoreService.subscribeToEmployees(userSession, company.companyId, setEmployees);
    const unsubApprovals = FirestoreService.subscribeToApprovalRequests(userSession, company.companyId, setApprovals);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-indigo-50/50 dark:bg-indigo-900/10 p-6 rounded-2xl border border-indigo-100 dark:border-indigo-800/30">
        <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-2">Regional Summary</h3>
