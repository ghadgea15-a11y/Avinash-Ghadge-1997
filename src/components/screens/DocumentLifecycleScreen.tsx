...
export const DocumentLifecycleScreen: React.FC<Props> = ({ session, userSession, activeCompany, onNavigate }) => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      default: return <span className="px-2 py-1 bg-slate-100 text-slate-700 text-xs font-bold rounded-lg">{status}</span>;
    }
  };

  if (loading) return <div className="p-8 text-center text-slate-500">Loading Document Lifecycle...</div>;
    ...
    <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          <div className="flex justify-between items-start">
            <div>
              ...
              <h3 className="text-2xl font-bold text-slate-900 mt-1">{data.documents.length}</h3>
            </div>
            <FileText className="w-8 h-8 text-slate-500 opacity-20" />
          </div>
        </div>
        <div className="bg-white p-6 rounded-2xl border border-emerald-200 bg-emerald-50/30 shadow-sm">
          <div className="flex justify-between items-start">
            <div>
      ...
      <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden flex flex-col min-h-[500px]">
        <div className="p-4 border-b border-slate-200 bg-slate-50 flex items-center justify-between">
          <h2 className="font-bold text-slate-900">Document Inventory</h2>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-100">
              {data.documents.length === 0 ? (
                <tr>
                  <td colSpan={5} className="p-8 text-center text-slate-500">No documents found. Run the E2E test to generate sample data.</td>
                </tr>
              ) : (
                data.documents.map((doc: any) => (
                  <tr key={doc.id} className="hover:bg-slate-50 transition-colors">
                    <td className="p-4">
                      ...
                      <div className="text-xs text-slate-500 mt-0.5">ID: {doc.id}</div>
                    </td>
                    <td className="p-4">
                      <span className="px-2 py-1 bg-slate-100 text-slate-700 text-[10px] font-bold rounded-md">
                        {doc.docType}
                      </span>
                    </td>
