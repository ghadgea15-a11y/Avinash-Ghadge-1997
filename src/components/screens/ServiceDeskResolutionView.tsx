  ...
  return (
    <div className="space-y-4 pt-1">
      ...
      {ticket.status !== 'RESOLVED' && ticket.status !== 'CLOSED' && (
        <div className="p-5 rounded-2xl border border-dashed border-slate-300 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/30 text-center space-y-3">
        ...
        <div className="space-y-4">
          ...
          <div className={`p-5 rounded-2xl border ${
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-750 space-y-1.5 md:col-span-2">
              ...
              {/* Root Cause Analysis */}
              <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-750 space-y-1.5">
                <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 flex items-center gap-1">
            ...
            {/* SLA Metrics Bar */}
            <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-750 grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
              <div>
                <span className="text-slate-400 text-[10px] uppercase font-semibold">Category</span>
                <p className="font-bold text-slate-800 dark:text-slate-200">{activeResolution.resolutionCategory || ticket.category}</p>
              </div>
              <div>
            ...
            {canVerify && activeResolution.verificationStatus !== 'VERIFIED' && (
              <div className="pt-3 border-t border-slate-200 dark:border-slate-700/60 flex flex-col sm:flex-row items-center justify-between gap-3">
                <div className="text-xs text-slate-500 dark:text-slate-400">
                  <span className="font-semibold text-slate-700 dark:text-slate-300">Quality Gate:</span> Verify that operational readiness is restored before client sign-off.
                </div>
            ...
            {isVerifying && (
              <div className="p-3 bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800 rounded-xl space-y-2 text-xs">
                ...
                <input
                  ...
                  className="w-full px-3 py-1.5 text-xs rounded-lg border border-emerald-300 dark:border-emerald-700 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200 focus:outline-hidden"
                />
              ...
              {showHistory && (
                <div className="space-y-3 pt-2">
                  {resolutions.slice(1).map((res, idx) => (
                    <div key={res.id} className="p-3 rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-850 text-xs space-y-1.5">
                      ...
                      <p className="text-slate-800 dark:text-slate-200">{res.resolutionSummary}</p>
                      {res.rejectionReason && (
              ...
              {showReopenHistory && (
                <div className="space-y-2.5 pt-2">
                  {reopenRecords.map((rec) => (
                    <div key={rec.id} className="p-3 rounded-lg border border-rose-200 dark:border-rose-900/40 bg-white dark:bg-slate-850 text-xs space-y-1.5">
                      ...
                      <div className="p-2 bg-slate-50 dark:bg-slate-900 rounded text-slate-700 dark:text-slate-300">
                        ...
                        <p>{rec.reason}</p>
                        {rec.notes && (
                          <p className="mt-1 text-slate-500 text-[11px] italic">Notes: {rec.notes}</p>
                        )}
                      </div>
                      <div className="flex flex-wrap items-center gap-3 text-[10px] text-slate-400">
                        <span>Reopened by: <strong className="text-slate-600 dark:text-slate-300">{rec.reopenedByName}</strong> ({rec.reopenedByRole})</span>
                        {rec.assignedToName && <span>Assigned to: <strong className="text-slate-600 dark:text-slate-300">{rec.assignedToName}</strong></span>}
            ...
            <form onSubmit={handleReject} className="space-y-3 text-xs">
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-3 py-2 text-xs rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200 focus:ring-2 focus:ring-rose-500 focus:outline-hidden resize-none"
                  required
                />
              </div>
