        ...
        {step === 'COMPANY_CODE' ? (
          ...
          <form onSubmit={handleLogin} className="space-y-3">
             <div className="text-center mb-6 space-y-1">
               <h2 className="text-lg font-bold">{validatedCompany?.brandName || 'Sign In'}</h2>
               <div className="flex items-center justify-center gap-2">
                 <span className={`text-xs font-mono px-2 py-0.5 rounded-full ${isDark ? 'bg-slate-800 text-slate-300' : 'bg-slate-100 text-slate-600'}`}>
                   {validatedCompany?.companyId}
                 </span>
                 <button 
                   type="button" 
                   onClick={() => {
                     ...
                     setCompanyCode('');
                     setError(null);
                     // Clear active company from session if they choose to change
                     SessionManager.clearActiveCompany(); 
                     // Not clearing fully, just letting them re-enter code
                   }}
