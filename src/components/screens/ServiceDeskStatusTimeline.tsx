...
export const ServiceDeskStatusTimeline: React.FC<ServiceDeskStatusTimelineProps> = ({
  ...
  const getStatusBadge = (status: ServiceTicketStatus | string) => {
    ...
    return (
      <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-bold border ${def.badgeBg} ${def.badgeText} ${def.badgeBorder}`}>
        {def.name}
      </span>
    );
  };
      ...
      <div className="p-4 bg-white dark:bg-slate-950 dark:bg-slate-800/60 rounded-xl border border-slate-200 dark:border-slate-700/60">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex items-center gap-2">
            {currentNorm === 'ON_HOLD' && (
              <span className="px-2 py-0.5 text-[11px] font-bold rounded-full bg-orange-100 text-orange-800 dark:bg-orange-950/60 dark:text-orange-300 animate-pulse flex items-center gap-1">
                <PauseCircle className="w-3 h-3" /> SLA Paused
              </span>
            )}
            {currentNorm === 'REOPENED' && (
              <span className="px-2 py-0.5 text-[11px] font-bold rounded-full bg-rose-100 text-rose-800 dark:bg-rose-950/60 dark:text-rose-300 flex items-center gap-1">
                <RotateCcw className="w-3 h-3" /> Lifecycle Reopened
              </span>
            )}
            {currentNorm === 'CANCELLED' && (
              <span className="px-2 py-0.5 text-[11px] font-bold rounded-full bg-red-100 text-red-800 dark:bg-red-950/60 dark:text-red-300 flex items-center gap-1">
              ...
              >
                ...
                <div className="flex items-center gap-2 text-xs text-slate-600 dark:text-slate-400 dark:text-slate-300">
                  ...
                  {record.changedByRole && (
                    <span className="px-1.5 py-0.2 text-[11px] rounded bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 font-mono">
                      {record.changedByRole}
                    </span>
                  )}
                </div>
                ...
                {record.pauseReason && (
                  <div className="p-2 rounded-lg bg-orange-50 dark:bg-orange-950/40 border border-orange-200 dark:border-orange-900 text-xs text-orange-800 dark:text-orange-300 flex items-center justify-between">
                    ...
                    <span className="text-[11px] px-1.5 py-0.5 bg-orange-200 dark:bg-orange-900 text-orange-900 dark:text-orange-100 rounded font-bold">
                      SLA Clock Paused
                    </span>
                  </div>
                )}
                  ...
                  <div className="p-2.5 rounded-lg bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-900 text-xs text-emerald-900 dark:text-emerald-200 space-y-1">
                    <div className="flex items-center justify-between font-bold">
                      ...
                      {record.resolutionCategory && (
                        <span className="text-[11px] px-1.5 py-0.5 bg-emerald-200 dark:bg-emerald-900 text-emerald-900 dark:text-emerald-100 rounded">
                          {record.resolutionCategory}
                        </span>
                      )}
                    </div>
