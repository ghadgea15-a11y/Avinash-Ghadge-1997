      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden my-8 max-h-[90vh] flex flex-col">
        ...
        <div className="p-6 overflow-y-auto space-y-5 flex-1 text-sm text-slate-800 dark:text-slate-200">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-xs">
            <div className="p-2.5 bg-slate-50 dark:bg-slate-800/30 rounded border border-slate-200 dark:border-slate-700">
              <span className="text-slate-500 dark:text-slate-400 block">Entity / Subject</span>
              <span className="font-semibold text-slate-900 dark:text-white">{violation.entityName || violation.entityId}</span>
            </div>
            <div className="p-2.5 bg-slate-50 dark:bg-slate-800/30 rounded border border-slate-200 dark:border-slate-700">
              <span className="text-slate-500 dark:text-slate-400 block">Entity Type</span>
              <span className="font-semibold text-slate-900 dark:text-white">{violation.entityType}</span>
            </div>
            <div className="p-2.5 bg-slate-50 dark:bg-slate-800/30 rounded border border-slate-200 dark:border-slate-700">
              <span className="text-slate-500 dark:text-slate-400 block">Site / Unit</span>
              <span className="font-semibold text-slate-900 dark:text-white">{violation.siteId || 'Global'}</span>
            </div>
            <div className="p-2.5 bg-slate-50 dark:bg-slate-800/30 rounded border border-slate-200 dark:border-slate-700">
              <span className="text-slate-500 dark:text-slate-400 block">Risk Score</span>
              <span className="font-semibold text-slate-900 dark:text-white">{violation.riskScore} / 100</span>
            </div>
            <div className="p-2.5 bg-slate-50 dark:bg-slate-800/30 rounded border border-slate-200 dark:border-slate-700">
            ...
            <div className="p-2.5 bg-slate-50 dark:bg-slate-800/30 rounded border border-slate-200 dark:border-slate-700">
              ...
              <span className="font-semibold text-slate-900 dark:text-white">{violation.bpmStatus || 'NONE'}</span>
            </div>
          </div>
          ...
          {violation.status !== 'RESOLVED' && violation.status !== 'FALSE_POSITIVE' && (
            <div className="space-y-4 pt-2 border-t border-slate-200 dark:border-slate-700">
              ...
              {/* Action Form */}
              {activeAction !== 'NONE' && (
                <div className="space-y-3 bg-slate-50 dark:bg-slate-800/40 p-4 rounded-lg border border-slate-200 dark:border-slate-700">
                  <div>
        ...
        <div className="px-6 py-3 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 flex items-center justify-end">
          <button
            type="button"
            onClick={onClose}
            className="px-4 py-2 text-xs font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg"
          >
            Close
          </button>
        </div>
