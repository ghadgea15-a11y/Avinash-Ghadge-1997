...
export const CreateBankBatchModal: React.FC<CreateBankBatchModalProps> = ({
  ...
  const handleCreateBatch = async () => {
    ...
    try {
      ...
      const result = await FirestoreService.createPaymentBatch(companyId, newBatch, {
        uid: session.userId,
        name: session.fullName || session.email || 'Finance Admin'
      });

      if (result.success) {
        onSuccess(result.batchId);
      } else {
        setErrorMessage(result.error || 'Failed to create payment batch.');
      }
    } catch (err: any) {
      ...
      <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/80">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-slate-900">Prepare New NEFT / RTGS Payment Batch</h3>
            ...
            <>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3.5 py-2.5 text-sm bg-slate-50 border border-slate-200 rounded-xl font-medium text-slate-900 focus:outline-hidden focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500"
                >
                ...
                <div className="p-4 bg-slate-50 rounded-xl border border-slate-200/80 space-y-3">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <div className="p-2.5 bg-white rounded-lg border border-slate-200">
                      <span className="text-[10px] text-slate-500 font-semibold uppercase">Eligible</span>
                      <p className="text-base font-bold text-emerald-600">{summary.totalValid}</p>
                    </div>
                    <div className="p-2.5 bg-white rounded-lg border border-slate-200">
                      <span className="text-[10px] text-slate-500 font-semibold uppercase">Issues / Duplicates</span>
                      <p className="text-base font-bold text-rose-600">{summary.totalInvalid}</p>
                    </div>
                    <div className="p-2.5 bg-white rounded-lg border border-slate-200">
                      <span className="text-[10px] text-slate-500 font-semibold uppercase">Batch Total</span>
                      <p className="text-base font-bold text-slate-900">₹{summary.totalAmount.toLocaleString('en-IN')}</p>
                    </div>
                  </div>
