  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setError(res.error || 'Failed to record export in audit logs');
      }
    } catch (err: any) {
      ...
      <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-3xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/80">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-slate-900">Secure NEFT / RTGS Bank File Export</h3>
              <p className="text-xs text-slate-500">
                Batch: <span className="font-mono font-medium text-slate-700">{batch.batchNumber}</span> • {batch.payrollCycleLabel}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-6 space-y-6 max-h-[75vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="p-4 bg-slate-50 rounded-xl border border-slate-200/80">
              <p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Total Disbursement</p>
              <p className="text-xl font-bold text-slate-900 mt-1">₹{batch.totalAmount.toLocaleString('en-IN')}</p>
            ...
            <div className="p-4 bg-slate-50 rounded-xl border border-slate-200/80">
              <p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Valid Beneficiaries</p>
              <p className="text-xl font-bold text-slate-900 mt-1">{batch.validBeneficiaryCount} / {batch.beneficiaryCount}</p>
              <p className="text-[11px] text-slate-500 mt-0.5">Mode: {batch.paymentMethod}</p>
            </div>
            <div className="p-4 bg-slate-50 rounded-xl border border-slate-200/80">
              <p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Debit Bank Source</p>
              ...
              {[
                ...
                >
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-slate-900">{fmt.name}</span>
                    {selectedFormat === fmt.id && (
          ...
          <div className="p-3.5 bg-slate-50 rounded-xl border border-slate-200 flex items-center justify-between text-xs">
            <div className="flex items-center space-x-2 text-slate-600">
              ...
              <span>Deterministic Checksum: <span className="font-mono font-bold text-slate-800">{fileResult?.checksum || 'N/A'}</span></span>
            </div>
