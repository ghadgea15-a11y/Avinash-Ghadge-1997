  ...
  return (
    <div className="flex h-full flex-col space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-medium text-slate-900">Gate Passes</h3>
        <button 
          ...
          <tbody className="divide-y divide-slate-100">
            {passes.map(p => (
              <tr key={p.id} className="hover:bg-slate-50">
                <td className="px-4 py-3 font-medium text-slate-900">{p.passNumber}</td>
                <td className="px-4 py-3 text-slate-600">{p.passType}</td>
                <td className="px-4 py-3 text-slate-600">
                  {p.sourceLocationName || 'External'} &rarr; {p.destinationLocationName || 'External'}
                </td>
                <td className="px-4 py-3 text-slate-600">{p.lines.length} lines</td>
            ...
            {passes.length === 0 && (
              <tr><td colSpan={6} className="text-center py-8 text-slate-500">No gate passes found</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      <div className="w-full max-w-2xl rounded-xl bg-white shadow-xl">
        <form onSubmit={handleSubmit}>
          <div className="border-b border-slate-200 px-6 py-4">
            <h3 className="text-lg font-semibold text-slate-900">Create Gate Pass</h3>
          </div>
          <div className="p-6 space-y-4">
            <div className="grid grid-cols-3 gap-4">
              ...
              <div>
                <label className="mb-1 block text-sm font-medium text-slate-700">Source Loc</label>
              ...
              <div>
                <label className="mb-1 block text-sm font-medium text-slate-700">Dest Loc</label>
