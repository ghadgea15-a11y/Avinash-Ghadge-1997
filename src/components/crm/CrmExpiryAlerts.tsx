...
export const CrmExpiryAlerts: React.FC<Props> = ({ session, company, contracts }) => {
  ...
  if (loading) {
    return <div className="text-center text-slate-500 py-4 animate-pulse">Loading Expiry Alerts...</div>;
  }

  if (events.length === 0) {
    return (
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-8 flex flex-col items-center justify-center text-slate-500">
        <CheckCircle2 className="w-12 h-12 text-emerald-300 mb-3" />
        <p className="font-medium text-slate-700">No active expiry alerts.</p>
    ...
    <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
      ...
      <div className="divide-y divide-slate-100">
        {events.slice(0, 10).map(evt => (
          <div key={evt.id} className="p-4 hover:bg-slate-50 flex items-start justify-between">
