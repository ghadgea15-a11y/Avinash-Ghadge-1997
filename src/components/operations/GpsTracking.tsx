...
export function GpsTracking({ session, sites, employees, selectedSiteId }: GpsTrackingProps) {
  ...
  // Note: In a real app, active sessions would be queried differently (e.g. all active sessions for a site).
  // For this demo, we'll simulate starting/stopping a session for the current user.
  
  // We'll use a simple polling/timeout to simulate GPS movement when a session is active.
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden min-h-[400px] relative flex flex-col">
          ...
          <div className="flex-1 p-6 flex flex-col items-center justify-center bg-slate-50/50">
             ...
             {!activeSession ? (
                ...
                <div className="w-full max-w-md bg-white border border-slate-200 p-4 rounded-lg shadow-sm">
                   <h4 className="font-medium text-slate-800 mb-4 border-b pb-2">Session Details</h4>
                   <div className="space-y-2 text-sm text-slate-600">
                      <p className="flex justify-between"><span>Session ID:</span> <span className="font-mono text-xs">{activeSession.id}</span></p>
                      <p className="flex justify-between"><span>Purpose:</span> <span className="px-2 py-1 rounded-full text-xs font-semibold border border-slate-200 text-slate-600 bg-white">{activeSession.purposeType}</span></p>
                   ...
                   {locationEvents.length > 0 && (
                     <div className="mt-6 pt-4 border-t border-slate-100">
                       <p className="text-xs text-slate-500 mb-2">Latest Coordinate</p>
                       <div className="bg-slate-50 p-3 rounded text-center font-mono text-xs text-slate-700">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col max-h-[600px]">
          <div className="px-4 py-3 border-b border-slate-100 bg-slate-50 font-medium text-slate-800">
            Event Log
          </div>
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {locationEvents.length === 0 ? (
               <p className="text-sm text-slate-500 text-center py-8">Waiting for location events...</p>
            ) : (
