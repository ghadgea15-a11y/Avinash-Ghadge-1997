...
export const SafetyChecksheetModule: React.FC<SafetyChecksheetModuleProps> = ({
  ...
  const renderBrandingHeader = (branding: { companyName: string, logoUrl?: string }) => (
    <div className="flex items-center justify-between border-b-2 border-slate-900 pb-4 mb-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-xl font-black text-slate-900 uppercase tracking-tight">{branding.companyName}</h2>
          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Official Safety Compliance Document</p>
        </div>
      </div>
      <div className="text-right">
        <div className="text-xs font-bold text-slate-400">Document ID</div>
        <div className="text-sm font-mono font-bold text-slate-900">SF-CHK-{Date.now().toString().slice(-6)}</div>
      </div>
    </div>
  );
  ...
  return (
    <div className="h-full flex flex-col bg-slate-50 dark:bg-slate-900 overflow-hidden">
      {/* Header */}
      <div className="bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 px-4 py-3 flex items-center justify-between flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Compliance & Audit Module</p>
          </div>
        </div>
        {view === 'LIST' && (
          <div className="flex items-center gap-2">
            <div className="relative">
              ...
              <input 
                ...
                className="pl-9 pr-4 py-1.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-xs w-48"
              />
            </div>
          </div>
        )}
      ...
      <div className="flex-1 overflow-y-auto p-4 lg:p-6">
        <div className="max-w-5xl mx-auto">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {TEMPLATES.map(temp => (
                  <button
                    ...
                    className="p-4 bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 hover:border-indigo-500 transition-all text-left shadow-sm hover:shadow-md group"
                  >
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{temp.category}</span>
                    </div>
                    <h3 className="font-bold text-slate-900 dark:text-white mb-1">{temp.title}</h3>
                    <p className="text-[10px] text-slate-500 font-medium">Create a new inspection report for this category.</p>
                  </button>
                ))}
              </div>
          ...
          {view === 'CREATE' && (
            <div className="max-w-3xl mx-auto bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
              {/* Branding Section */}
              <div className="p-8 border-b border-slate-100 dark:border-slate-700">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mt-6">
                  <div className="flex-1">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">Audit Site / Location</label>
                    <select
                  ...
                  <div className="w-full md:w-48 text-right">
                    <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Inspection Date</div>
                    <div className="p-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-bold flex items-center justify-end gap-2">
              ...
              <div className="p-8 space-y-8">
                <div className="space-y-4">
                  {formItems.map((item, idx) => (
                    <div key={item.id} className="p-4 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-100 dark:border-slate-700 flex flex-col md:flex-row md:items-center justify-between gap-4">
                      <div className="flex-1">
                        <span className="text-[10px] font-bold text-indigo-500 uppercase tracking-widest">{item.category}</span>
                        <p className="text-sm font-bold text-slate-900 dark:text-white mt-1">{item.question}</p>
                      </div>
                      <div className="flex items-center gap-1 p-1 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                        ...
                        <button 
                          ...
                          className={`px-4 py-2 rounded-lg text-[10px] font-black transition ${
                            item.response === 'NA' ? 'bg-slate-500 text-white shadow-lg' : 'text-slate-400 hover:text-slate-600'
                          }`}
                        >
                          N/A
                        </button>
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Overall Summary & Corrective Actions</label>
                  <textarea
                    rows={4}
                    placeholder="Enter inspection summary, identified risks, or required corrective actions..."
                    className="w-full p-4 rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-sm focus:ring-2 focus:ring-indigo-500 outline-none"
              ...
              {/* Footer */}
              <div className="p-8 bg-slate-50 dark:bg-slate-900/50 border-t border-slate-100 dark:border-slate-700 flex items-center justify-between">
          ...
          {view === 'DETAIL' && selectedRecord && (
            <div className="max-w-3xl mx-auto bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-xl overflow-hidden print:shadow-none print:border-none">
              <div className="p-8 border-b border-slate-100 dark:border-slate-700">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mt-8">
                  <div className="space-y-4 flex-1">
                    <h2 className="text-2xl font-black text-slate-900 dark:text-white uppercase leading-tight">{selectedRecord.title}</h2>
                    <div className="flex flex-wrap gap-4">
                      <div className="flex items-center gap-2 px-3 py-1.5 bg-slate-100 dark:bg-slate-900 rounded-lg">
                        <MapPin className="w-3.5 h-3.5 text-indigo-500" />
                        <span className="text-[10px] font-bold text-slate-700 dark:text-slate-300 uppercase">{selectedRecord.siteName}</span>
                      </div>
                      <div className="flex items-center gap-2 px-3 py-1.5 bg-slate-100 dark:bg-slate-900 rounded-lg">
                        <User className="w-3.5 h-3.5 text-indigo-500" />
                        <span className="text-[10px] font-bold text-slate-700 dark:text-slate-300 uppercase">{selectedRecord.performedByUserName}</span>
                      </div>
                    </div>
                  </div>
                  <div className="flex flex-col items-center justify-center p-6 bg-slate-50 dark:bg-slate-950 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-800">
                    <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Overall Result</div>
              ...
              <div className="p-8">
                <div className="space-y-4">
                  {selectedRecord.items.map((item) => (
                    <div key={item.id} className="flex items-start justify-between py-3 border-b border-slate-50 dark:border-slate-700 last:border-0">
                      ...
                      <div className={`px-4 py-1 rounded-md text-[10px] font-black border ${
                        item.response === 'YES' 
                          ? 'bg-emerald-50 text-emerald-600 border-emerald-100' 
                          : item.response === 'NO'
                          ? 'bg-rose-50 text-rose-600 border-rose-100'
                ...
                {selectedRecord.summaryRemarks && (
                  <div className="mt-8 p-6 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-100 dark:border-slate-700">
                ...
                <div className="mt-12 pt-8 border-t-2 border-slate-100 dark:border-slate-700 flex justify-between items-end">
                  <div className="space-y-1">
                    ...
                    <div className="text-[10px] font-bold text-slate-500 uppercase">Authorized Safety Officer Signature</div>
                  </div>
                  <div className="text-right">
                    <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Report Generated</div>
              ...
              <div className="p-8 bg-slate-50 dark:bg-slate-900/50 border-t border-slate-100 dark:border-slate-700 flex items-center justify-between print:hidden">
                <button onClick={() => setView('LIST')} className="text-xs font-bold text-slate-500 hover:text-slate-700 flex items-center gap-2">
