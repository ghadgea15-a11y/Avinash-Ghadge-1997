...
export const PrivilegeMatrixViewer: React.FC<PrivilegeMatrixViewerProps> = ({ userSession }) => {
  ...
  const targetSession = useMemo<UserSession>(() => {
    return {
      ...
      dataScope: RbacService.getDataScope({ ...userSession, role: selectedRole } as any, 'ALL')
    };
    ...
    <div id="privilege-governance-matrix" className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 shadow-sm">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-semibold text-black dark:text-white">Privilege Governance & Access Control Matrix</h2>
            </div>
          ...
          <div className="flex items-center gap-3">
            <label className="text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wider">Inspect Role:</label>
            <select
              value={selectedRole}
              onChange={(e) => setSelectedRole(e.target.value as UserRole)}
              className="px-3 py-2 bg-white dark:bg-slate-950 border border-slate-300 rounded-lg text-sm font-medium text-black dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500"
            >
              {ALL_ROLES.map(role => (
        ...
        <div className="mt-4 pt-4 border-t border-slate-100 grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs">
          <div className="bg-white dark:bg-slate-950 p-3 rounded-lg border border-slate-100">
            <span className="text-slate-500 dark:text-slate-400 block mb-1">Effective Authority Level</span>
            <span className="font-bold text-black dark:text-slate-200 text-sm">{targetSession.authorityLevel}</span>
          </div>
          <div className="bg-white dark:bg-slate-950 p-3 rounded-lg border border-slate-100">
            <span className="text-slate-500 dark:text-slate-400 block mb-1">Canonical Data Scope</span>
            <span className="font-bold text-black dark:text-slate-200 text-sm">{targetSession.dataScope}</span>
          </div>
          <div className="bg-white dark:bg-slate-950 p-3 rounded-lg border border-slate-100">
            <span className="text-slate-500 dark:text-slate-400 block mb-1">Active Tenant Boundary</span>
            <span className="font-bold text-indigo-700 text-sm truncate block">{targetSession.companyId}</span>
          </div>
          <div className="bg-white dark:bg-slate-950 p-3 rounded-lg border border-slate-100">
            <span className="text-slate-500 dark:text-slate-400 block mb-1">Assigned Site / Branch</span>
            <span className="font-bold text-black dark:text-slate-200 text-sm truncate block">{targetSession.assignedSiteId || targetSession.branchId || 'Enterprise'}</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 rounded-lg text-sm text-black dark:text-slate-200 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500"
          />
        </div>
        ...
        <select
          ...
          className="px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 rounded-lg text-sm text-slate-900 dark:text-slate-300 focus:outline-none focus:ring-2 focus:ring-indigo-500"
        >
                ...
                return (
                  <tr 
                    ...
                    className={`hover:bg-white transition-colors ${
                      isGranted ? 'bg-white' : 'bg-white/40 text-slate-400'
                    }`}
                  >
